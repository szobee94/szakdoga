    ...
    if message.topic == topic_receive:
        ...
        if request == "arucos":
            ...
            for i in range(1, len(splitted), 3):
                ...
                y = float(splitted[i + 1])
                z = float(splitted[i + 2])
                r, g, b = float(splitted[i + 3]), float(splitted[i + 4]), float(splitted[i + 5])
                try:
                    current_refpoints[(x, y, z, r, g, b)] += 1
                except KeyError:
                    current_refpoints[(x, y, z, r, g, b)] = 1
        ...
        if msg_id == 1:
            refpoints_received = []
            for i in range(1, len(message.payload), 15):
                ...
                r = np.frombuffer(message.payload[(i + 12):(i + 13)], dtype=np.uint8)[0]
                g = np.frombuffer(message.payload[(i + 13):(i + 14)], dtype=np.uint8)[0]
                b = np.frombuffer(message.payload[(i + 14):(i + 15)], dtype=np.uint8)[0]
                try:
                    current_refpoints[(x, y, z, r, g, b)] += 1
                except KeyError:
                    current_refpoints[(x, y, z, r, g, b)] = 1
                    refpoints_received.append([x, y, z, r, g, b])
...
def save_refpoints():
    ...
    save_file.write("pos_x,pos_y,pos_z,color_r,color_b,color_b\n")
    for index in current_refpoints:
...
def load_refpoints(save_name: str):
    ...
    with open(source, 'r') as csv_file:
        ...
        for row in csv_reader:
            if row_count > 0:
                x, y, z, r, g, b = float(row[0]), float(row[1]), float(row[2]), \
                                   float(row[3]), float(row[4]), float(row[5])
                if len(row) > 6:
                    aggr_num = int(row[6])
                    current_refpoints[(x, y, z, r, g, b)] = aggr_num
                else:
                    current_refpoints[(x, y, z, r, g, b)] = 1
            row_count += 1
